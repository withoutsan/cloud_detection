...
# from losses import BMMKD
from timm.data import AugMixDataset, create_dataset, create_loader, FastCollateMixup, Mixup, resolve_data_config
from timm.loss import *

from timm.models import convert_splitbn_model, create_model, load_checkpoint, model_parameters, resume_checkpoint, \
    safe_model_name
from timm.optim import create_optimizer_v2, optimizer_kwargs
from timm.scheduler import create_scheduler
from timm.utils import *
from timm.utils import ApexScaler, NativeScaler
import network.ghostnetv3_pytorch.ghostnetv3 as ghostnetv3
...
# KD parameters
parser.add_argument('--teacher', default='beitv2_large_patch16_224', type=str)
parser.add_argument('--teacher-pretrained', default='', type=str)
parser.add_argument('--ori-loss-weight', default=1., type=float)
...
parser.add_argument('--eval-interval', type=int, default=1)  # eval every 1 epochs before epochs * eval_interval_end
...
parser.add_argument('--width', type=float, default=1.3, help='model width (default: 1.3)')
...
def main():
    ...
    def kd_loss(z_s, z_t):
        log_prob_s = F.log_softmax(z_s, dim=1).unsqueeze(2)
        prob_t = F.softmax(z_t, dim=1).unsqueeze(1)
        kd_loss = -torch.bmm(prob_t, log_prob_s)
    ...
    if args.rank == 0:
        ...
        if model_ema is not None:
            ema_saver_dir = os.path.join(output_dir, 'ema_checkpoint')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                epoch, model, teacher, loader_train, optimizer, train_loss_fn, kd_loss_fn, args,
            ...
            if epoch > int(args.eval_interval_end * args.epochs) or epoch % args.eval_interval == 0:
                ...
                if model_ema is not None and not args.model_ema_force_cpu:
                    ...
                    if ema_saver is not None:
                        # save proper checkpoint with eval metric
                        save_metric = eval_metrics[eval_metric]
                        best_metric, best_epoch = ema_saver.save_checkpoint(epoch, metric=save_metric)
...
def train_one_epoch(
        epoch, model, teacher, loader, optimizer, loss_fn, kd_loss_fn, args,
        lr_scheduler=None, saver=None, output_dir=None, amp_autocast=suppress, loss_scaler=None,
        model_ema=None, mixup_fn=None, teacher_resizer=None, student_resizer=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            loss_ori = args.ori_loss_weight * loss_fn(output, target)
            loss_kd = args.kd_loss_weight * kd_loss_fn(z_s=output, z_t=output_t)
            loss = loss_ori + loss_kd

        if not args.distributed:
            losses_m.update(loss.item(), input.size(0))
            losses_ori_m.update(loss_ori.item(), input.size(0))
            losses_kd_m.update(loss_kd.item(), input.size(0))
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                losses_ori_m.update(reduced_loss_ori.item(), input.size(0))
                losses_kd_m.update(reduced_loss_kd.item(), input.size(0))
