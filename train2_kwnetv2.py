...
from network.KernelWarehouse.kw1d2x_mobilenetv2 import kw1d2x_mobilenetv2
...
if __name__ == "__main__":
    ...
    snapshot_path = snapshot_path + '_' + 'KW_MobileNetV2'
    if not os.path.exists(snapshot_path):
        os.makedirs(snapshot_path)

    # 模型
    # model = CDCTFM(img_size=args.img_size, num_classes=2, in_channel=4).cuda()
    model = kw1d2x_mobilenetv2(img_size=args.img_size, num_classes=2, in_channel=10).cuda()
    ...
    for epoch_num in iterator:
        ...
        if epoch_num % 5 == 0 and epoch_num >= 80:
